...
import { User, MapPin, ChevronDown, Droplets, X, Mail, Lock, ChevronLeft } from 'lucide-react-native';
...
export default function RegisterScreen({ onSignIn, onBack }: Props) {
  ...
  const handleRegister = async () => {
    ...
    if (password.length < 6) {
      showAlert({ type: 'error', title: 'Weak Password', message: 'Password must be at least 6 characters long' });
      return;
    }
    ...
    try {
      ...
      if (result.data.token && result.data.user) {
        ...
        showAlert({ type: 'error', title: 'Error', message: 'Registration failed' });
      }
    } catch (err: any) {
