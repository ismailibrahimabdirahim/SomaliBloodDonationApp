...
import { ArrowLeft, Droplets, MapPin, Phone, Heart, ShieldCheck, Palette, Check, X, MessageCircle, Video } from 'lucide-react-native';
...
export default function PublicProfileScreen({ route, navigation }: Props) {
  ...
  const handleCall = async () => {
    ...
    const callChannel = [user?.email || '', email].sort().join('__').replace(/[^a-zA-Z0-9_]/g, '_').slice(0, 64);
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.actionRow}>
          <TouchableOpacity style={[styles.actionBtn, { backgroundColor: colors.card }]} onPress={handleCall} activeOpacity={0.7}>
            <Video size={24} color={palette.bloodRed} />
            <Text style={[styles.actionText, { color: palette.bloodRed }]}>Video Call</Text>
          </TouchableOpacity>
          <TouchableOpacity
