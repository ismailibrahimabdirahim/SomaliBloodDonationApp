...
export default function RequestDetailScreen({ route, navigation }: Props) {
  ...
  const [proofImage, setProofImage] = useState(String(requestData.proofImage ?? ''));
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      await api.put(`/api/requests/${id}`, { name, phone, location, type: bloodType, urgency, description, proofImage });
                ...
                <>
                  ...
                  {!!proofImage && (
                    <View style={[styles.proofCard, { backgroundColor: colors.card, borderColor: colors.border }]}> 
                      <Text style={[styles.fieldLabel, { color: colors.textMuted }]}>MEDICAL PROOF</Text>
