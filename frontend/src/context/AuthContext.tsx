...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const [isLoaded, setIsLoaded] = useState(true); // Start as loaded - only show loading if we have a user to restore
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        if (savedUser) {
          // Only show loading screen if we have a user to restore
          setIsLoaded(false);
        ...
        // If no saved user, isLoaded stays true - no loading screen
      } catch (e) {
